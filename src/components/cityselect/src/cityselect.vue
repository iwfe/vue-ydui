    ...
    <div class="m-cityselect" :class="show ? 'cityselect-active' : ''">
      <div class="cityselect-header">
        ...
        <div v-show="ready" class="cityselect-nav">
          <a href="javascript:;" v-show="!!nav.txt1" @click.stop="backoffView(0)" :class="nav.index == 0 ? 'cityselect-nav-active' : ''">{{nav.txt1}}</a>
          <a href="javascript:;" v-show="!!nav.txt2" @click.stop="backoffView(1)" :class="nav.index == 1 ? 'cityselect-nav-active' : ''">{{nav.txt2}}</a>
          <a href="javascript:;" v-show="!!nav.txt3" @click.stop="forwardView(true)" :class="nav.index == 2 ? 'cityselect-nav-active' : ''">{{nav.txt3}}</a>
        </div>
      </div>
      ...
      <ul v-show="ready" class="cityselect-content" :class="activeClasses">
        <li class="cityselect-item" ref="provanceBox">
          <div class="cityselect-item-box">
            <a href="javascript:;" :class="item.n == activeProvance ? 'cityselect-item-active' : ''" v-for="item in citys" @click.stop="chooseProvance(item.n, item.c)">
              <span>{{item.n}}</span>
            </a>
          </div>
        </li>
        <li class="cityselect-item" ref="cityBox">
          <div class="cityselect-item-box">
            <a href="javascript:;" :class="item.n == activeCity ? 'cityselect-item-active' : ''" v-for="item in cityArray" @click.stop="chooseCity(item.n, item.a)">
              <span>{{item.n}}</span>
            </a>
          </div>
        </li>
        <li class="cityselect-item" ref="areaBox">
          <div class="cityselect-item-box">
            <a href="javascript:;" :class="item == activeArea ? 'cityselect-item-active' : ''" v-for="item in areaArray" @click.stop="chooseArea(item)">
              <span>{{item}}</span>
            </a>
          </div>
        </li>
